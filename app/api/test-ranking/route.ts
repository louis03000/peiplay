...
export async function POST() {
  try {
    ...
    // 創建測試資料
    const { partner, partner2, customer } = await db.query(async (client) => {
      // 創建測試夥伴
      const partnerUser = await client.user.create({
        data: {
      ...
      // 創建測試客戶
      const customerUser = await client.user.create({
        data: {
      ...
      const customerData = await client.customer.create({
        data: {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        // 創建上午時段
        const morningSchedule = await client.schedule.create({
          data: {
        ...
        // 創建下午時段
        const afternoonSchedule = await client.schedule.create({
          data: {
            partnerId: partnerData.id,
            date: date,
      ...
      // 創建第二個夥伴（較少的預約）
      const partnerUser2 = await client.user.create({
        data: {
      ...
      const partner2Data = await client.partner.create({
        data: {
      ...
      return { partner: partnerData, partner2: partner2Data, customer: customerData };
    });
