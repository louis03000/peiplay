...
// 完全按照綠界官方範例的 CheckMacValue 計算方式
...
export async function GET() {
  try {
    ...
    // 使用綠界官方範例格式的訂單編號
    const orderNumber = `ECPay${now.getTime()}`
    ...
    const officialExactHtml = `
      <!DOCTYPE html>
      <html>
      <head>
        <title>綠界官方確切範例測試</title>
        <meta charset="UTF-8">
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>📋 綠界官方確切範例測試</h1>
            <p style="color: #666; font-size: 1.2em;">完全按照綠界官方範例的參數和計算方式</p>
          </div>
          
          <div class="alert alert-info">
            <strong>🎯 測試目標：</strong> 使用綠界官方確切範例參數
          </div>
          ...
          <div class="section">
            ...
            <div class="step">
              ...
              <br><strong>格式：</strong> ECPay + 時間戳記（綠界官方範例格式）
          ...
          <div class="section">
            ...
            <form method="POST" action="${ECPAY_CONFIG.PAYMENT_URL}" target="_blank">
              ...
              <button type="submit" class="test-button">📋 官方確切範例測試</button>
            </form>
          </div>
          
          <div class="alert alert-success">
            <strong>✅ 使用綠界官方確切範例參數：</strong> 
            <ul>
              <li>訂單編號格式：ECPay + 時間戳記（官方範例格式）</li>
              <li>參數完全按照官方範例：ChoosePayment, EncryptType, ItemName, MerchantID, MerchantTradeDate, MerchantTradeNo, PaymentType, ReturnURL, TotalAmount, TradeDesc</li>
              <li>CheckMacValue 計算：HashKey + 參數 + HashIV → URL encode → 轉小寫 → SHA256</li>
            </ul>
          </div>
          
          <div class="alert alert-danger">
            <strong>⚠️ 如果還是失敗：</strong> 問題可能是綠界後台配置，請聯繫綠界客服確認：
            <ul>
          ...
          <div class="alert alert-warning">
            <strong>💡 重要提醒：</strong> 
            <ul>
              <li>此測試使用綠界官方確切範例的參數格式</li>
              <li>如果還是失敗，問題在綠界後台配置</li>
              <li>請聯繫綠界客服確認商店設定</li>
            </ul>
          </div>
        </div>
    ...
    return new NextResponse(officialExactHtml, {
      headers: {
    ...
    console.error('Official exact error:', error)
