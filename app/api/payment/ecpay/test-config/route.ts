    ...
    const debugHtml = `
      ...
      <head>
        ...
        <style>
          ...
          .config { background: #e8f4f8; padding: 10px; }
        </style>
      </head>
      <body>
        <h1>綠界配置測試</h1>
        ...
        <div class="section">
          <h2>常見問題檢查</h2>
          <ul>
            <li>MerchantID 是否正確: <span class="${ECPAY_CONFIG.MERCHANT_ID === '3464691' ? 'success' : 'error'}">${ECPAY_CONFIG.MERCHANT_ID}</span></li>
            ...
            <li>訂單編號格式: <span class="${orderNumber.length <= 20 ? 'success' : 'error'}">${orderNumber.length} 字元</span></li>
            <li>金額格式: <span class="${ecpayParams.TotalAmount === '100' ? 'success' : 'error'}">${ecpayParams.TotalAmount}</span></li>
          </ul>
        </div>
      </body>
      </html>
