...
export async function GET() {
  try {
    ...
    // 使用最短的訂單編號：只有 12 字元
    const orderNumber = `E${year}${month}${day}${hour}${minute}${second}${random}`
    ...
    const ecpayExampleHtml = `
      <!DOCTYPE html>
      <html>
      <head>
        <title>綠界官方範例測試</title>
        <meta charset="UTF-8">
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>📚 綠界官方範例測試</h1>
            <p style="color: #666; font-size: 1.2em;">使用綠界官方範例參數</p>
          </div>
          
          <div class="alert alert-info">
            <strong>🎯 測試目標：</strong> 使用綠界官方範例參數（最簡化）
          </div>
          ...
          <div class="section">
            ...
            <div class="step">
              ...
              <br><strong>格式：</strong> E + 年月日時分秒 + 1位隨機數
          ...
          <div class="section">
            ...
            <form method="POST" action="${ECPAY_CONFIG.PAYMENT_URL}" target="_blank">
              ...
              <button type="submit" class="test-button">📚 範例測試</button>
            </form>
          </div>
          
          <div class="alert alert-success">
            <strong>✅ 使用綠界官方範例參數：</strong> 
            <ul>
              <li>訂單編號長度：${orderNumber.length} 字元</li>
              <li>只包含必要參數：MerchantID, MerchantTradeNo, MerchantTradeDate, PaymentType, TotalAmount, TradeDesc, ItemName, ReturnURL, ChoosePayment, EncryptType</li>
          ...
          <div class="alert alert-warning">
            <strong>💡 建議：</strong> 如果所有測試都失敗，請：
            <ul>
              <li>聯繫綠界客服確認商店配置</li>
              <li>確認是否在測試環境還是正式環境</li>
              <li>檢查商店是否已通過審核</li>
              <li>確認付款方式是否已啟用</li>
            </ul>
          </div>
    ...
    return new NextResponse(ecpayExampleHtml, {
      headers: {
    ...
    console.error('ECPay example error:', error)
